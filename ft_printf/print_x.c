...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/05/23 13:59:38 by nseniak           #+#    #+#             */
...
int	print_x_4(int min, int zero, int count, int left_justify)
{
	int		i;
	char	to_print;
...
void	print_x_3(int n, int precision, int x)
{
	int	count;
	int	n_len;
	...
	print_hexa_lower((unsigned int)n);
}

int	print_x_2(char *str, int n, int min, int zero)
{
	int	i;
	int	count;
	int	precision;
	...
	if (zero && ft_strchrn(str, '#', i) && n)
		ft_putstr_fd("0x", 1);
	if (left_justify)
		print_x_3(n, precision, !zero && ft_strchrn(str, '#', i));
	count += print_x_4(min, zero, count, left_justify);
	if (!left_justify)
		print_x_3(n, precision, !zero && ft_strchrn(str, '#', i));
	return (count);
}
...
int	print_x_1(char *str, int i, va_list args)
{
	...
	return (print_x_2(str, va_arg(args, int), min, zero));
}
